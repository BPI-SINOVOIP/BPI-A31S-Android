...
COMMOM_PARAMS:
	...
	// add for CTS
	if (mCameraConfig->supportViewAngle())
	{
		value = mCameraConfig->getHorizontalViewAngle();
		p.set(CameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, value);

		value = mCameraConfig->getVerticalViewAngle();
		p.set(CameraParameters::KEY_VERTICAL_VIEW_ANGLE, value);
	}
	else
	{
		if (mHalCameraInfo.facing == CAMERA_FACING_BACK)
