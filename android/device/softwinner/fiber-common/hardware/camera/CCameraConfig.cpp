...
void CCameraConfig::initParameters()
{
	...
	memcpy(mUsedViewAngle, "0\0", 2);
	mHorizontalViewAngle = 0;
	mVerticalViewAngle = 0;
	if(readKey(kUSED_VIEW_ANGLE, mUsedViewAngle))
	{
		if (usedKey(mUsedViewAngle))
		{
			READ_KEY_VALUE(kHORIZONTAL_VIEW_ANGLE, mHorizontalViewAngle);
			READ_KEY_VALUE(kVERTICAL_VIEW_ANGLE, mVerticalViewAngle);
		}
		else
		{
...
bool CCameraConfig::supportViewAngle()
{
	return usedKey(mUsedViewAngle);	
}

char * CCameraConfig::getHorizontalViewAngle()
{
	return mHorizontalViewAngle;
}

char * CCameraConfig::getVerticalViewAngle()
{
