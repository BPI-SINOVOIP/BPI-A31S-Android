...
static void select_output_device(struct sunxi_audio_device *adev)
{
    ...
    ALOGV("select_output_device, devices: %x, mode: %x, select_out_device: %x", adev->hw_device, adev->mode, adev->out_device);
    ...
    if (adev->mode == AUDIO_MODE_NORMAL){
        ...
        normal_play_route(AUDIO_DEVICE_OUT_WIRED_HEADPHONE);
    ...
    for (index = 0; index < MAX_AUDIO_DEVICES; index++)
    {
            ...
            && adev->dev_manager[index].flag_out_active)
        {
			...
			if(!strncmp(adev->dev_manager[index].name, "AUDIO_USB", 9)){
				ALOGD("use usb audio out to playback audio.");
                         ...
                         size_t bytes)
{
    ...
    for (index = MAX_AUDIO_DEVICES; index >= 0; index--)
    {
            ...
            && adev->dev_manager[index].flag_out_active)
        {
            ...
            if (strncmp(adev->dev_manager[index].name, "AUDIO_USB", 9))
            {
                //ALOGV("pcm_mmap_write card: %d", card);
            ...
            if(ret!=0){
                ALOGE("##############out_write()  Warning:write fail, reopen it#######################");
                do_output_standby(out);
                break;
            }
        }
