            ...
            while (max-- > 0) {
                try {
                    ...
                    while (timeout > 0) {
                         ...
                         */
                        if (response.received && request.questionType == response.recordType && response.addrEntry[0] != null) {
                            log.print("response.addrEntry[0].address = " +response.addrEntry[0].getHostAddress());
                            int i = 0;
                            for (i =0; i < SambaManager.index; i++) {
                                if(response.addrEntry[0].getHostAddress().equals(SambaManager.badAddress.get(i))) {
                                    log.print("address equals = " + response.addrEntry[0].getHostAddress());
                                    break;
                                }
                            }
                            if(i == SambaManager.index)
                                return;
                        }
                    ...
                    case RESOLVER_BCAST:
                        ...
                        while( n-- > 0 ) {
                            ...
                            if( response.received && response.resultCode == 0 ) {
                                ...
                                log.print("response.addrEntry[0] = " + response.addrEntry[0].getHostAddress());
