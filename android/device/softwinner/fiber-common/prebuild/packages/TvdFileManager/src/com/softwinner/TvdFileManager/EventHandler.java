...
public class EventHandler implements OnClickListener,OnItemSelectedListener,OnItemClickListener, OnItemLongClickListener
{
	...
	public EventHandler(Context context,
			final IEventHandlerCallbacks mCallbacks, MediaProvider mProvider,
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				switch (msg.what) {
				case PICTURE:
					Bitmap bm = mMedia.getImageThumbFromMK(picPath,
							imageThumb.getWidth(), imageThumb.getHeight());
	...
	public void setViewResource(SurfaceView video, ImageView image,
			TextView preview, TextView path, TextView index) {
	...
	private void MapPartitionName(String devices) {
		...
		String[] map = {
				mContext.getResources().getString(R.string.partitionA),
	...
	public void onItemClick(final AdapterView<?> parent, final View view,
		...
		/* 如果点中的是"网络邻居",或者点击的是samba服务器,或者点击的是samba的Share文件夹,则开始搜索 */
		...
		else if(mNfs.isNfsShare(filePath))
		{
			...
			if (mountResule) {
				...
				Toast.makeText(mContext, mContext.getText(R.string.try_mount_fail), Toast.LENGTH_LONG).show();
			}
	...
	private String findBlurayVideo(String path) {
		...
		for (String st : list) {
			...
			if (file.isFile() && TypeFilter.isMovieFile(st)) {
				...
				if (itg != null)
					Log.d("chen", "integer  " + itg.intValue());
	...
	private void playIso(String path) {
		...
		Log.d(TAG, "fileManager__playIso__realpath: " + realpath);
		String cdromPath = mIso.getVirtualCDRomPath(path);
		Log.d(TAG, "fileManager__playIso__cdromPath: " + cdromPath);
		if (cdromPath != null) {
	...
	private void showDeviceMessage(String path) {
		...
		try {
			...
			String Target = mContext.getResources().getString(R.string.target)
					+ target + "\n";
			String Usedsize = mContext.getResources().getString(
					R.string.used_size)
					+ usedSize + "\n";
			String Availsize = mContext.getResources().getString(
					R.string.avail_size)
					+ availSize + "\n";
		...
		if (c != null) {
			try {
				while (c.moveToNext()) {
					...
					Singer = mContext.getResources().getString(R.string.singer)
							+ c.getString(ARTIST_INDEX) + "\n";
					Title = mContext.getResources().getString(R.string.title)
							+ c.getString(TITLE_INDEX) + "\n";
					long time = Integer.valueOf(c.getString(DURATION_INDEX))
							.longValue();
					Duration = mContext.getResources().getString(
							R.string.duration)
							+ toDuration(time) + "\n";
					Size = mContext.getResources().getString(R.string.size)
							+ toSize(new File(path).length()) + "\n";
				}
			} finally {
				c.close();
				c = null;
	...
	private void getPicDetail(String path) {
		...
		String Size = mContext.getResources().getString(R.string.size) + s
				+ "\n";
		...
		String Resolution = mContext.getResources().getString(
				R.string.resolution)
				+ wPic + "*" + hPic + "\n";
	...
	private void showIndex(int position) {
		...
		String index = String.valueOf(position + 1) + "/"
				+ String.valueOf(mDataSource.size());
	...
	public String getDeviceSelectedPath(){
		String path = null;
		if(currentDir == rootPath && currentPosition >= 0 && mDataSource.size() > currentPosition)
		{
			path = mDataSource.get(currentPosition);
		}
		return path;
	}
	...
	public void deleteFile(String name) {
		 ...
		 */
		if (name.equals(MediaProvider.RETURN)
				|| mDevices.isLocalDevicesRootPath(name)
				|| mDevices.hasMultiplePartition(name.substring(0,
						name.lastIndexOf("/")))) {
			return;
		}
		/*
		...
		if (!f.canWrite()) {
			DisplayToast(mContext.getResources().getString(
					R.string.operate_fail_dueto_permission));
			return;
		}
	...
	public void copyFile(String oldLocation, String newLocation) {
		...
		if (newLocation.contains(oldLocation)) {
			if (delete_after_copy) {
				msg = mContext.getResources().getString(R.string.can_not_cut)
						+ name;
			} else {
				msg = mContext.getResources().getString(R.string.can_not_copy)
						+ name;
			}
			if (newLocation.equals(oldLocation)) {
				msg = msg
						+ mContext.getResources().getString(
								R.string.target_equals_src);
			} else {
				msg = msg
						+ mContext.getResources().getString(
								R.string.target_is_child);
			}
			DisplayToast(msg);
			return;
		}
		...
		File newFile = new File(newDir, name);
           ...
           if (newFile.exists()) {
                   ...
                   builder.setPositiveButton(convice,
                                   new DialogInterface.OnClickListener() {
                                           public void onClick(DialogInterface dialog, int which) {
		...
		if (newFile.exists()
				...
				&& ((newFile.isDirectory() && old.isDirectory()) || (newFile
						.isFile() && old.isFile()))) {
			AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
			// builder.setTitle(mContext.getResources().getString(R.string.Warning));
			// builder.setIcon(R.drawable.warning);
		...
		if (!old.exists()) {
			DisplayToast(mContext.getResources().getString(
					R.string.operate_no_exit_file));
		}

		if (!old.canRead() || (delete_after_copy && !old.canWrite())
				|| !newDir.canWrite()) {
			DisplayToast(mContext.getResources().getString(
					R.string.operate_fail_dueto_permission));
			return;
	...
	public class TableRow extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (mDevices.isInterStoragePath(path)) {
			...
			} else if (path.equals(mContext.getResources().getString(
					R.string.samba))) {
				holder.text.setText(path);
				...
				if (file.isDirectory()) {
					if (mDevices.isLocalDevicesRootPath(file.getParent())) {
						...
						if (tmp.equals(path)) {
							holder.icon
									.setImageResource(R.drawable.litter_file);
						} else {
							fileName = tmp;
							holder.icon
									.setImageResource(R.drawable.litter_partition);
						}
					} else {
	...
	public class MenuItemListener implements OnClickListener {
		...
		private void saveCopyFilePath(String filePath) {
			...
			try {
				BufferedWriter o_stream = new BufferedWriter(new FileWriter(f,
						true));
	...
	private void selectFileType_dialog() {
		...
		String modifiedTime = mContext.getResources().getString(
				R.string.modified_time);
		...
		builder.setItems(type, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				ProgressDialog dg = ProgressDialog.show(mContext, null,
						mContext.getResources().getString(R.string.sorting));
				sorting();
				mTable.notifyDataSetChanged();
	...
	private void rename(final String path) {
		 ...
		 */
		if (path.equals(MediaProvider.RETURN)
				|| mDevices.isLocalDevicesRootPath(path)) {
			return;
		}
		/* 文件权限不足时，也不能改名 */
		File f = new File(path);
		...
		rename_create.setText(mContext.getResources()
		...
		rename_create.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				int ret = operate.renameTarget(path, rename_input.getText()
						.toString());
				switch (ret) {
				case -1:
					DisplayToast(mContext.getResources().getString(
				...
				case 0:
					...
					String newPath = dirPath + "/"
							+ rename_input.getText().toString() + ext;
					Log.d(TAG, newPath);
	...
	private class BackgroundWork extends
		...
		protected void onPreExecute() {
			...
			switch (type) {
			case SEARCH_TYPE:
				showProgressDialog(R.drawable.icon, mContext.getResources()
						.getString(R.string.hello), null,
						ProgressDialog.STYLE_SPINNER, true);
				break;
			case COPY_TYPE:
				String title;
				title = mContext.getResources().getString(
						R.string.copy_to_newDir);
				showProgressDialog(R.drawable.icon, title, null,
						ProgressDialog.STYLE_HORIZONTAL, true);
				break;
			case DELETE_TYPE:
				showProgressDialog(R.drawable.icon, mContext.getResources()
						.getString(R.string.delete_files), null,
						ProgressDialog.STYLE_HORIZONTAL, true);
				break;
			case SCANFILES:
		...
		private void showProgressDialog(int icon, String title, String message,
				int style, final boolean cancelable) {
		...
		protected ArrayList<String> doInBackground(String... params) {
			...
			case COPY_TYPE:
				copyFile = params[0];
				newFile = params[1]
						+ copyFile.substring(copyFile.lastIndexOf("/"));
		...
		protected void onProgressUpdate(Integer... values) {
			...
			if (i >= 0) {
				...
				case DELETE_OLD_FILES:
					pr_dialog.setTitle(mContext.getResources().getString(
							R.string.delete_old_files));
					break;
				case DELAY_SCAN:
					if (isScanning) {
						showProgressDialog(R.drawable.icon, mContext
								.getResources().getString(R.string.scan), null,
								ProgressDialog.STYLE_SPINNER, false);
					}
					break;
				}
