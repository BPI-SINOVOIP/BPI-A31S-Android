...
public class VideoView extends SurfaceView implements MediaPlayerControl,
	...
	private void setvideoSize(int width, int height) {
		...
		Log.d(TAG, "#################setvideoSize(), result: width = " + width
				+ ", height = " + height);
		setMeasuredDimension(width, height);
	}
	...
	public void setTimedTextPath(ArrayList<String> srtList,
			ArrayList<String> mediaTypeList) {
		Log.v("setTimedTextPath",
				"setTimedTextPathsetTimedTextPathsetTimedTextPath");
		mSrtList = srtList;
		...
		try {
			...
			if (mSrtList != null && mSrtList.size() > 0) {
				for (int j = 0; j < mSrtList.size(); j++) {
					 ...
					 */
					mMediaPlayer.addTimedTextSource(mSrtList.get(j),
							mMediaTypeList.get(j));
					Log.v("maizirong", "here j = " + j + ", mSrtList.get(j) = "
							+ mSrtList.get(j) + ", mMediaTypeList.get(j) = "
							+ mMediaTypeList.get(j));
				}
			}
			...
			if (mTrackInfosAudio.size() != 0) {
				if (mTrackInfosAudio.size() > mTrackAudioCur
						&& mTrackAudioCur >= 0) {
