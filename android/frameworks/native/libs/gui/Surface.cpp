...
int Surface::queueBuffer(android_native_buffer_t* buffer, int fenceFd) {
    ...
    IGraphicBufferProducer::QueueBufferInput input(timestamp, isAutoTimestamp,
            ...
            mVideoAddr0, mVideoAddr1, mVideoAddr2, mVideoAddr3, mVideoAddr4,
            mVideoAddr5, mVideoFormat, mInterlace, mTopFieldFirst);
...
int Surface::perform(int operation, va_list args)
{
    ...
    case NATIVE_WINDOW_API_DISCONNECT:
        ...
        break;
    case NATIVE_WINDOW_GET_OUTPUT_TYPE:
        res = getVideoOutputType();
        break;
    case NATIVE_WINDOW_SET_VIDEO_BUFFERS_INFO:
        res = dispatchSetVideoBuffersInfo(args);
        break;
    case NATIVE_WINDOW_SET_DIT_INFO:
        res = dispatchSetDitInfo(args);
        break;
    default:
        res = NAME_NOT_FOUND;
        break;
...
int Surface::dispatchSetVideoBuffersInfo(va_list args) {
    uint32_t viderAddr0 = va_arg(args, uint32_t);
    uint32_t viderAddr1 = va_arg(args, uint32_t);
    uint32_t viderAddr2 = va_arg(args, uint32_t);
    uint32_t viderAddr3 = va_arg(args, uint32_t);
    uint32_t viderAddr4 = va_arg(args, uint32_t);
    uint32_t viderAddr5 = va_arg(args, uint32_t);
    uint32_t format = va_arg(args, uint32_t);
    return SetVideoBuffersInfo(viderAddr0, viderAddr1, viderAddr2, viderAddr3, viderAddr4, viderAddr5, format);
}

int Surface::dispatchSetDitInfo(va_list args) {
    uint32_t interlace = va_arg(args, uint32_t);
    uint32_t topFieldFirst = va_arg(args, uint32_t);
    return SetDitInfo(interlace, topFieldFirst);
}
...
int Surface::getVideoOutputType()
{
    int ret = mGraphicBufferProducer->getVideoOutputType();

    return ret;
}

int Surface::SetVideoBuffersInfo(uint32_t videoAddr0, uint32_t videoAddr1, uint32_t videoAddr2, uint32_t videoAddr3, uint32_t videoAddr4, uint32_t videoAddr5, uint32_t format)
{
    Mutex::Autolock lock(mMutex);
    mVideoAddr0 = videoAddr0;
    mVideoAddr1 = videoAddr1;
    mVideoAddr2 = videoAddr2;
    mVideoAddr3 = videoAddr3;
    mVideoAddr4 = videoAddr4;
    mVideoAddr5 = videoAddr5;
    mVideoFormat= format;
    return NO_ERROR;
}

int Surface::SetDitInfo(uint32_t interlace,uint32_t topFieldFirst) {
    Mutex::Autolock lock(mMutex);
    mInterlace = interlace;
    mTopFieldFirst = topFieldFirst;
    return NO_ERROR;
}
