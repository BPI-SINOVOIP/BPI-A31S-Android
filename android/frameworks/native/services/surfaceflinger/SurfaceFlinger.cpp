...
void SurfaceFlinger::createBuiltinDisplayLocked(DisplayDevice::DisplayType type) {
    ...
    info.isSecure = mHwc->isSecure(type);
...
void SurfaceFlinger::bootFinished()
{
    ...
    char value[PROPERTY_VALUE_MAX];
    property_get("dev.disp_type", value, "-1");
    int type = atoi(value);
    property_get("dev.disp_mode", value, "-1");
    int mode = atoi(value);
    ALOGD("disp type %d , disp mode %d", type, mode);

#define DISPLAY_CMD_SETOUTPUTMODE 6

    if(type != -1 && mode != -1){
        setDisplayParameter(0, DISPLAY_CMD_SETOUTPUTMODE, type , mode, 0);
    }
}
...
void SurfaceFlinger::init() {
    ...
    getDefaultDisplayDevice()->makeCurrent(mEGLDisplay, mEGLContext, false);
    ...
    if (hwc.initCheck() == NO_ERROR) {
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            if (id >= 0) {
                ...
                for (size_t i=0 ; cur!=end && i<count ; ++i, ++cur) {
                    const sp<Layer>& layer(currentLayers[i]);
                    layer->setCompositionType(hw, *cur);
                }
            }
        }
            ...
            for (size_t i=0 ; i<dc ; i++) {
                ...
                if (j < 0) {
                    ...
                    if (!draw[i].isMainDisplay()) {
                        ...
                        defaultDisplay->makeCurrent(mEGLDisplay, mEGLContext,false);
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    if (hasGlesComposition) {
        if (!hw->makeCurrent(mEGLDisplay, mEGLContext,mforcemakecurrent)) {
            ALOGW("DisplayDevice::makeCurrent failed. Aborting surface composition for display %s",
                  hw->getDisplayName().string());
	        mforcemakecurrent = true;
            return;
        } else{
	        mforcemakecurrent = false; 
	    }
...
int SurfaceFlinger::getDisplayParameter(int displayId, int cmd, int para0, int para1) {
    const HWComposer& hwc(getHwComposer());
    return hwc.getDisplayParameter(displayId, cmd, para0, para1);
}
