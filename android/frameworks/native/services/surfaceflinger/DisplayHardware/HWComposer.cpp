...
status_t HWComposer::queryDisplayProperties(int disp) {
    ...
    for (size_t i = 0; i < NUM_DISPLAY_ATTRIBUTES - 1; i++) {
        ...
        case HWC_DISPLAY_IS_SECURE:
            mDisplayData[disp].isSecure = values[i];
            break;
        default:
...
bool HWComposer::isSecure(int disp) const {
    return mDisplayData[disp].isSecure;
}
...
public:
    ...
    virtual void setBuffer(const sp<GraphicBuffer>& buffer) {
        if (buffer == 0 || buffer->handle == 0) {
            ...
            getLayer()->videoAddr0 = buffer->mVideoAddr0;
            getLayer()->videoAddr1 = buffer->mVideoAddr1;
            getLayer()->videoAddr2 = buffer->mVideoAddr2;
            getLayer()->videoAddr3 = buffer->mVideoAddr3;
            getLayer()->videoAddr4 = buffer->mVideoAddr4;
            getLayer()->videoAddr5 = buffer->mVideoAddr5;
            getLayer()->videoFormat = buffer->mVideoFormat;
            getLayer()->interlace = buffer->mInterlace;
            getLayer()->topFieldFirst = buffer->mTopFieldFirst;
        }
    }
    ...
    virtual uint32_t getVideoFormat()
    {
        return getLayer()->videoFormat;
    }
        ...
        for (size_t i=0 ; i<mNumDisplays ; i++) {
            ...
            if (disp.list) {
                ...
                for (size_t i=0 ; i<disp.list->numHwLayers ; i++) {
                    ...
                    if (hwcHasApiVersion(mHwc, HWC_DEVICE_API_VERSION_1_3)) {
                        result.appendFormat(
                                " %10s | %08x | %08x | %08x | %02x | %05x | %08x | %08x | %08x | %08x |[%7.1f,%7.1f,%7.1f,%7.1f] | [%5d,%5d,%5d,%5d] %s\n",
                                        compositionTypeName[type],
                                        intptr_t(l.handle), l.hints, l.flags, l.transform, l.blending, format, l.videoFormat,
                                        l.interlace, l.topFieldFirst,
                                        l.sourceCropf.left, l.sourceCropf.top, l.sourceCropf.right, l.sourceCropf.bottom,
                                        l.displayFrame.left, l.displayFrame.top, l.displayFrame.right, l.displayFrame.bottom,
                                        name.string());
                    } else {
                        result.appendFormat(
                                " %10s | %08x | %08x | %08x | %02x | %05x | %08x | %08x | %08x | %08x | [%7d,%7d,%7d,%7d] | [%5d,%5d,%5d,%5d] %s\n",
                                        compositionTypeName[type],
                                        intptr_t(l.handle), l.hints, l.flags, l.transform, l.blending, format, l.videoFormat,
                                        l.interlace, l.topFieldFirst,
...
int HWComposer::getDisplayParameter(int disp, int cmd, int para0, int para1) const {
    if (mHwc && mHwc->getDisplayParameter) {
        return mHwc->getDisplayParameter(mHwc, disp, cmd, para0, para1);
    }

    return NO_INIT;
}
int HWComposer::setDisplayParameter(int disp, int cmd, int para0, int para1, int para2) const {
    if (mHwc && mHwc->setDisplayParameter) {
        return mHwc->setDisplayParameter(mHwc, disp, cmd, para0, para1, para2);
    }
