...
static void* MainThread(void* arg)
{
	...
	//AALOGD("mFrameRate = %d", t->mFrameRate);
	...
	//AALOGD("open decoder return %d", ret);
        	...
        	else
        	{
        		while(1)
        		{
        			if(decoderLocked == 0)
        			{
        				//* lock decoder.
        				ve_mutex_lock(&t->mCedarVRequestCtx);
                        //AALOGD("(f:%s l:%d) TPlayer", __FUNCTION__, __LINE__);
        				decoderLocked = 1;
        			}

        			decodeResult = t->mDecoder->decode(t->mDecoder);
                    //AALOGD("(f:%s l:%d) TPlayer", __FUNCTION__, __LINE__); //when hdmi plugin and plugout, thumbplayer will ...
    				if(decoderLocked == 1)
    				{
    					//* unlock decoder.
    					ve_mutex_unlock(&t->mCedarVRequestCtx);
