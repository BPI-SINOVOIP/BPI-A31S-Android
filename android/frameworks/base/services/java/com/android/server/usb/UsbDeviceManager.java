...
public class UsbDeviceManager {
    ...
    // add for boofast mode usb detect.
    private PowerManager mPM;
    private int mPlugType;
    private int mBootFastRef=0;
    ...
    public UsbDeviceManager(Context context) {
        ...
        wl = mPM.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, TAG);
    ...
    private final class UsbHandler extends Handler {
		...
		private final BroadcastReceiver mBootFastReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent){
                ...
                if(boot==1){
                    mHandler.obtainMessage(MSG_BOOTFAST_SWITCHED, 1, 0).sendToTarget();
                    mBootFastRef=0;
                }else{
                    if(mBootFastRef==0){
                        mHandler.obtainMessage(MSG_BOOTFAST_SWITCHED, 0, 0).sendToTarget();
                        mBootFastRef++;
                    }
               }
			}
        ...
        private final BroadcastReceiver mBatteryServiceReceiver = new BroadcastReceiver(){
            @Override
            public void onReceive(Context context, Intent intent){
                Slog.d(TAG,"mBootFastReceiver reveived ACTION_BATTERY_CHANGE");
                final int oldPlugType = mPlugType;
                mPlugType = intent.getIntExtra(BatteryManager.EXTRA_PLUGGED, 1);
                if(mPlugType != oldPlugType){
                    Slog.d(TAG,"connect status chaged");
                    if(mPM.isBootFastStatus()){
                        if((mPlugType&BatteryManager.BATTERY_PLUGGED_ANY)!=0){
                            Slog.d(TAG,"boot fast status connect");
                            setUsbConfig(mCurrentFunctions);
							SystemClock.sleep(5000);
							if(mPM.isBootFastStatus());
							setUsbConfig("none");
                        }else{
                            Slog.d(TAG,"boot fast status disconnect");
                        }
                    }
                } 
            }
        ...
        public UsbHandler(Looper looper) {
            ...
            try {
                ...
                mContext.registerReceiver(
                        mBatteryServiceReceiver, new IntentFilter(Intent.ACTION_BATTERY_CHANGED));
            } catch (Exception e) {
