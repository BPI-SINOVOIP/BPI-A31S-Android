...
public class ConnectivityService extends IConnectivityManager.Stub {
    ...
    public NetworkInfo getActiveNetworkInfo() {
        ...
        final ContentResolver cr = mContext.getContentResolver();
        if (Settings.Secure.getInt(cr, Settings.Secure.WIFI_CHEAT_ON, 0) != 0) {
            ...
            if((info != null) && (info.getType() == ConnectivityManager.TYPE_ETHERNET)) {
                ...
                notTrueWifi.setDetailedState(info.getDetailedState(), info.getReason(), info.getExtraInfo());
                notTrueWifi.setIsAvailable(info.isAvailable());
                log("getActiveNetworkInfo: wifi disconnectd, eth connected, cheated."); 
                log("cheat wifiinfo: " + notTrueWifi);
                return notTrueWifi;
            }
        } 
    ...
    public NetworkInfo getNetworkInfo(int networkType) {
        ...
        if (Settings.Secure.getInt(cr, Settings.Secure.WIFI_CHEAT_ON, 0) != 0) {
            ...
            if((info != null)
                ...
                info.setDetailedState(ethInfo.getDetailedState(), ethInfo.getReason(), ethInfo.getExtraInfo());
                info.setIsAvailable(ethInfo.isAvailable());
                log("getNetworkInfo: wifi disconnected, eth connected, cheated.");
                return info;
            }
        }
        //Cheat ethernet to wifi --End--
...
//                if (currentPriority >= mNetConfigs[checkType].mPriority) continue;
    ...
    private void handleConnect(NetworkInfo info) {
        ...
        if (VDBG) {
            log("handleConnect: newNetType=" + newNetType + " thisIface=" + thisIface
    ...
    private boolean updateRoutes(LinkProperties newLp, LinkProperties curLp,
        ...
        for (RouteInfo r :  routeDiff.added) {
            if (isLinkDefault || ! r.isDefaultRoute()) {
                ...
                // Wifi/Eth coexist strategy V0.3
                if (mActiveDefaultNetwork == ConnectivityManager.TYPE_ETHERNET 
                        && newLp.getInterfaceName().contains("wlan")) {
                    Slog.d(TAG, "Ethernet already connected, wifi connected after, can't update default route.");
                    continue;
                }
                Slog.d(TAG, "mActiveDefaultNetwork:" + mActiveDefaultNetwork);
                Slog.d(TAG, "newLp.InterfaceName:" + newLp.getInterfaceName());
                // End
        ...
        if (nt != null && nt.getNetworkInfo().isConnected() && !nt.isTeardownRequested()) {
            ...
            if (mNetConfigs[netType].isDefault()) {
                ...
                synchronized (mDnsLock) {
                    // Wifi/Eth coexist strategy V0.3
                    Slog.d(TAG, "handleDnsConfigurationChange: netType(" + netType + ")");
                    Slog.d(TAG, "dnses:" + dnses);
                    if (mActiveDefaultNetwork == ConnectivityManager.TYPE_ETHERNET 
                            && netType == ConnectivityManager.TYPE_WIFI) {
                        Slog.d(TAG, "Ethernet already connected, Wifi connected after, can't set wifi as default dns");
