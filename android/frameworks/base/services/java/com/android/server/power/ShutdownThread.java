...
public final class ShutdownThread extends Thread {
	...
	private static String mPrivateApps[] = 
    ...
    private void setAirplaneModeState(boolean enabled) {
		...
		if(Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON,0)==1){
	 	...
	 	Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON_FAST_BOOT,1);
    ...
    public void run() {
		...
		if(mWindowManager != null){
			try{
                //mWindowManager.freezeRotation(mWindowManager.getRotation());
                boolean alreadyfreeze = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.ACCELEROMETER_ROTATION, 0, UserHandle.USER_CURRENT) == 0;
				int direction = mWindowManager.getRotation();
                if(!alreadyfreeze || Surface.ROTATION_0 != direction) {
                	Log.d(TAG,"********freeze it**********");
                    //mWindowManager.freezeRotation(mWindowManager.getRotation());
                    mWindowManager.freezeRotation(Surface.ROTATION_0);
                }
				Settings.System.putInt(mContext.getContentResolver(), 
					     Settings.System.ROTATION_MODE_SET,alreadyfreeze?0:1);
				Settings.System.putInt(mContext.getContentResolver(), 
					     Settings.System.ROTATION_DIRECTION, direction);
       ...
       if(recents != null)
         {
         		...
         		for(int i=0;i<recents.size();i++)
         		{
                		...
                		if (task != null) 
                		{
                			...
                			{
                				String taskName = task.baseIntent.getComponent().getPackageName();
								if(taskName!=null)
                				if(!filterPrivateApps(taskName))
                				{
                    					am.removeTask(task.persistentId, ActivityManager.REMOVE_TASK_KILL_PROCESS);
                     				Log.v(TAG,"remove a task " + taskName);
                                    if(thumbs!=null)
                                    {
    ...
    private boolean filterPrivateApps(String name){
		for(int i=0;i<mPrivateApps.length;i++){
			Log.d(TAG,mPrivateApps[i] + "name = " + name);
			if(mPrivateApps[i].equals(name))
				return true;
		}
		return false;
    }
