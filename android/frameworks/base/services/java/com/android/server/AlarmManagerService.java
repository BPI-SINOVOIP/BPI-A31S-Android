...
import static android.app.AlarmManager.RTC_SUSPEND_WAKEUP;
import static android.app.AlarmManager.RTC_SHUTDOWN_WAKEUP;
...
class AlarmManagerService extends IAlarmManager.Stub {
    ...
    private static final int RTC_SHUTDOWN_WAKEUP_MASK = 1 << RTC_SHUTDOWN_WAKEUP;
    private static final int RTC_SUSPEND_WAKEUP_MASK = 1 << RTC_SUSPEND_WAKEUP;

    private static final int TIME_CHANGED_MASK = 1 << 16;
    private static final int IS_WAKEUP_MASK = RTC_WAKEUP_MASK|ELAPSED_REALTIME_WAKEUP_MASK|
                                                RTC_SHUTDOWN_WAKEUP_MASK|RTC_SUSPEND_WAKEUP_MASK;
    ...
    static final class Batch {
        ...
        Batch(Alarm seed) {
            ...
            if((seed.type==RTC_SHUTDOWN_WAKEUP)||(seed.type==RTC_SUSPEND_WAKEUP)){
                new_type = 1;
            }else{
                new_type = 0;
            }
    ...
    static long convertToElapsed(long when, int type) {
        final boolean isRtc = (type == RTC || type == RTC_WAKEUP || type == RTC_SUSPEND_WAKEUP || type == RTC_SHUTDOWN_WAKEUP);
        if (isRtc) {
    ...
    // Return the index of the matching batch, or -1 if none found.
    int attemptCoalesceLocked_wakeup(long whenElapsed, long maxWhen) {
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            if ((b.new_type==1) && (!b.standalone) && b.canHold(whenElapsed, maxWhen)) {
                return i;
            }
        }
        return -1;
    ...
    public void set(int type, long triggerAtTime, long windowLength, long interval,
        ...
        if (type < RTC_WAKEUP || type > RTC_SUSPEND_WAKEUP) {
    ...
    private void setImplLocked(int type, long when, long whenElapsed, long windowLength,
        ...
        if ((type==RTC_SUSPEND_WAKEUP)||(type==RTC_SHUTDOWN_WAKEUP)) {
            whichBatch = -1;
            isStandalone = true;
        } else {
    ...
    private Batch findFirstNonWakeupBatchLocked() {
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            if ((b.new_type!=1)) {
    ...
    private Batch findFirstWakeupBatchLocked_suspend() {
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            if (b.new_type==1) {
    ...
    private void rescheduleKernelAlarmsLocked() {
        ...
        if (mAlarmBatches.size() > 0) {
            ...
            final Batch firstSuspend = findFirstWakeupBatchLocked_suspend();
            final Batch firstBatch = findFirstNonWakeupBatchLocked();
            ...
            if (firstSuspend != null && mNextSuspend != firstSuspend.start) {
                mNextSuspend = firstSuspend.start;
                setLocked(RTC_SHUTDOWN_WAKEUP, firstSuspend.start);
            }
        }
    }
