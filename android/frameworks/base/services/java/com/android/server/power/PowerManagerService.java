...
public final class PowerManagerService extends IPowerManager.Stub
	...
	private long mEnterBootFastTime = 0;
	...
	private boolean goToBootFastSleepNoUpdateLocked(long eventTime) {
        ...
        mEnterBootFastTime = SystemClock.uptimeMillis();
		if(DEBUG_BOOTFAST)
            Slog.d(TAG, "mEnterBootFastTime = " + mEnterBootFastTime);

        return true;
	}

    public long getEnterBootFastTime(){
        return mEnterBootFastTime;
    }
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason) {
        ...
        if (!(eventTime < mLastWakeTime || mWakefulness == WAKEFULNESS_ASLEEP
                || !mSystemReady || mBootFastStats) && !mBootCompleted) {
    ...
    private void updateSuspendBlockerLocked() {
		...
		if(mIsPowered&&mBootFastStats){
			needDisplaySuspendBlocker = true;
			//mHoldingDisplaySuspendBlocker = true;
			Slog.d(TAG,"we need display lock");
		}else{
			needDisplaySuspendBlocker = needDisplaySuspendBlocker();
		}
		if(mBootFastStats){
			needWakeLockSuspendBlocker = false;
			Slog.d(TAG,"we not need wake lock");
		}else{
    ...
    /**
     * set battery chage state
     *
     * @param true enable charge false disable charge
     */
    @Override // Binder call
    public void setBatteryChargeState(boolean state) {
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.DEVICE_POWER, null);
        final long ident = Binder.clearCallingIdentity();
        try {
            mBatteryService.setChargeState(state);
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
    }
