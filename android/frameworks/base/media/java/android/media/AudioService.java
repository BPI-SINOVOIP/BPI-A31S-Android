...
public class AudioService extends IAudioService.Stub {
   ...
   /** @hide Maximum volume index values for audio streams */
	...
	private boolean mInFmMode = false;
    ...
    public void setStreamMute(int streamType, boolean state, IBinder cb) {
        boolean showUI = (streamType & AudioManager.SHOW_MUTE_UI) != 0;
        streamType &= ~AudioManager.SHOW_MUTE_UI;
        if (mUseFixedVolume) {
            return;
        }
    ...
    public void setMode(int mode, IBinder cb) {
		...
		if (!mInFmMode && mode == AudioSystem.MODE_FM) {
			setStandbyLock(true);
			mInFmMode = true;
		} else if (mInFmMode && mode != AudioSystem.MODE_FM) {
			setStandbyLock(false);
			mInFmMode = false;
		}
    }

	private void setStandbyLock(boolean lock){
	    Log.i(TAG, "setStandbyState(" + lock + ")");
		String sceneLockPath = "/sys/power/scene_lock";
		String sceneUnlockPath = "/sys/power/scene_unlock";

		try {
            FileWriter sceneLock = new FileWriter(sceneLockPath);
			FileWriter sceneUnlock = new FileWriter(sceneUnlockPath);
            if (lock) {
                sceneLock.write("talking_standby");
            } else {
                sceneUnlock.write("talking_standby");
            }
            sceneLock.close();
			sceneUnlock.close();
        } catch (IOException e){
    ...
    private class AudioServiceBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(Intent.ACTION_BOOT_COMPLETED) || action.equals(Intent.ACTION_UI_BOOT_FINISH)) {
	            if (!mBootCompleted) {
	                Log.d(TAG, "ACTION_BOOT_COMPLETED in AudioService.java");
	                mBootCompleted = true;
