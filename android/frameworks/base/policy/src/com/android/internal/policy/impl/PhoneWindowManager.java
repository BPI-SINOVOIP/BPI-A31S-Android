...
import android.hardware.display.DisplayManagerPolicy;
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private UEventObserver mHDMIObserver = new UEventObserver() {
        ...
        public void onUEvent(UEventObserver.UEvent event) {
            boolean hdmiPlugged = "1".equals(event.get("SWITCH_STATE"));
    ...
    private void interceptPowerKeyDown(boolean handled) {
        ...
        if (!handled) {
            ...
            if(shortPressLeadShutdown)
                ...
                delay = ViewConfiguration.getGlobalActionKeyTimeout();
    ...
    public void updateBootFastSettings(){       
        if(Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON,0)==1){
            if(Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.AIRPLANE_MODE_ON_FAST_BOOT,
                0)==1){
                Slog.d(TAG,"updateBootFastSettings close air mode");
        ...
        Settings.Global.putInt(mContext.getContentResolver(), 
                            Settings.Global.AIRPLANE_MODE_ON_FAST_BOOT,0);
        
        if(Settings.System.getInt(mContext.getContentResolver(),Settings.System.ACCELEROMETER_ROTATION, 0) == 0){
            int rotationSet = Settings.System.getIntForUser(mContext.getContentResolver(),Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT);
            Log.d(TAG,"rotationSet = "+rotationSet);

            if(rotationSet==1){
                try{
                    mWindowManager.thawRotation();
                }catch(Exception e){
                    Slog.e(TAG,"thawRotation failed");
                }
            }   
        }
        Settings.System.putIntForUser(mContext.getContentResolver(),
                            Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT);
    }
    ...
    private void setAirplaneModeState(boolean enabled) {
     // TODO: Sets the view to be "awaiting" if not already awaiting
        ...
        if(Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.AIRPLANE_MODE_ON_FAST_BOOT,0)==1){
            Slog.d(TAG,"user setting airplane mode is on");
            Settings.Global.putInt(mContext.getContentResolver(), Settings.Global.AIRPLANE_MODE_ON_FAST_BOOT,0);
                ...
                public void run(){
                    ...
                    try{
                        boolean alreadyfreeze = Settings.System.getIntForUser(mContext.getContentResolver(),
                                                    Settings.System.ROTATION_MODE_SET,0,UserHandle.USER_CURRENT)==0;
                        if (!alreadyfreeze) {
                            Log.d(TAG, "************thawRotation**************");
                            mWindowManager.thawRotation();
                        } else {
                            int direction = Settings.System.getIntForUser(mContext.getContentResolver(),
                                                    Settings.System.ROTATION_DIRECTION,0,UserHandle.USER_CURRENT);
                            if (Surface.ROTATION_0 != direction) {
                                mWindowManager.freezeRotation(direction);
    ...
    private final Runnable mPowerLongPress = new Runnable() {
        ...
        public void run() {
            ...
            case LONG_PRESS_POWER_GLOBAL_ACTIONS:
                ...
                /* modified by Gary. start {{----------------------------------- */
                /* 2013-03-16 */
                /* directly power off with no dialog */
                {
	...
	mAlwaysFullScreen = SystemProperties.getBoolean("ro.statusbar.alwayshide", false);
	mHasNavigationBar &= (!mAlwaysFullScreen);
    ...
    private void releaseBootAnimationView(){
        if (mBootAnimationView != null) {
            ...
            mBootAnimationView.BootAnimationViewRealease();
	...
	/* add by yuguoxu. to dealwith key in FactoryMode such as sheild some key,and start test tools */
	private boolean mFactoryKeyHandle = false;
	
	private void interceptFactoryKeyDown(int keyCode){
		Log.d(TAG,"interceptFactoryKeyDown mFactoryKeyHandle = " + mFactoryKeyHandle);
		if(!mFactoryKeyHandle){
			mFactoryKeyHandle = true;
			if(keyCode == KeyEvent.KEYCODE_5)
				mHandler.postDelayed(mFactoryKey5LongPress, 1000);
			else if(keyCode == KeyEvent.KEYCODE_1)
				mHandler.postDelayed(mFactoryKey1LongPress, 1000);
			else if(keyCode == KeyEvent.KEYCODE_9)
				mHandler.postDelayed(mFactoryKey9LongPress, 1000);
		}	
	}
	
	private void interceptFactoryKeyUp(int keyCode){
		Log.d(TAG,"interceptFactoryKeyUp mFactoryKeyHandle = " + mFactoryKeyHandle);
		if(mFactoryKeyHandle){
			if(keyCode == KeyEvent.KEYCODE_5)
				mHandler.removeCallbacks(mFactoryKey5LongPress);
			else if(keyCode == KeyEvent.KEYCODE_1)
				mHandler.removeCallbacks(mFactoryKey1LongPress);
			else if(keyCode == KeyEvent.KEYCODE_9)
				mHandler.removeCallbacks(mFactoryKey9LongPress);
			mFactoryKeyHandle = false;
		}
	}

	private final Runnable mFactoryKey5LongPress = new Runnable( ) {
		@Override
		public void run(){
			try{
				String TestPackageName = SystemProperties.get("ro.sw.testapkpackage", "no");
                String TestClassName = SystemProperties.get("ro.sw.testapkclass", "no");
				Log.d(TAG,"reset package " + TestPackageName); 
				String Action = "com.softwinner.dragonbox.ACTION_RESET";
	...
	private final Runnable mFactoryKey1LongPress = new Runnable( ) {
		@Override
		public void run(){
			try{
				String TestPackageName = SystemProperties.get("ro.sw.agingtestpackage", "no");
                String TestClassName = SystemProperties.get("ro.sw.agingtestclass", "no");
	...
	private final Runnable mFactoryKey9LongPress = new Runnable( ) {
		@Override
		public void run(){
			try{
				String TestPackageName = SystemProperties.get("ro.sw.snwritepackage", "no");
                String TestClassName = SystemProperties.get("ro.sw.snwriteclass", "no");
                Intent intent = new Intent();
                intent.setComponent(new ComponentName(TestPackageName,TestClassName));
				intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(intent);
	...
	private boolean dealwithKeyInFactoryMode(KeyEvent event){
		int keyCode = event.getKeyCode();
		final int flags = event.getFlags();
		final boolean down = event.getAction() == KeyEvent.ACTION_DOWN;
		String sheildKey = SystemProperties.get("persist.sw.sheildkey","176,3,4,183,184,185,186,66,67");
		Log.d(TAG,"sheildKey = " + sheildKey);
		String ssKeyCode[] = sheildKey.split(",");
		for(String key:ssKeyCode){
			if(Integer.parseInt(key) == keyCode){
				Log.d(TAG,"key " + KeyEvent.keyCodeToString(keyCode) + " is matched" );
				return true;
			}
		}
		if(down){
			interceptFactoryKeyDown(keyCode);
		}else{
			interceptFactoryKeyUp(keyCode);
		}
		return false;
	}

	private boolean isFactoryConfigExist(int flags){
		final int FLAG_DISMISS_HOMEKEY = 0x02000000;
		boolean retval = false;
		if((flags&FLAG_DISMISS_HOMEKEY)!=0){
			retval = true;
		}
		Log.d(TAG,"isFactoryConfigExist retval = " + retval);
		return retval;
	}
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
		...
		WindowManager.LayoutParams tattrs = win != null ? win.getAttrs() : null;
		if(isFactoryConfigExist(tattrs.flags)){
			if(dealwithKeyInFactoryMode(event)){
				return -1;
			}
		}
        ...
        if ((event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0 && keyCode == KeyEvent.KEYCODE_HOME
        ...
        else if (mAssistKeyLongPressed == true && keyCode == KeyEvent.KEYCODE_HOME
                 && ((policyFlags & WindowManagerPolicy.FLAG_VIRTUAL)==0)) {
            if (!keyguardOn) 
            {
                keyCode = KeyEvent.KEYCODE_ASSIST;
        ...
        /* add by Gary. start {{----------------------------------- */
        else if (keyCode == KeyEvent.KEYCODE_TV_SYSTEM) {
            if (!down) {
                DisplayManagerPolicy dmp = new DisplayManagerPolicy(mContext);
                dmp.showNextDevice();
            }
            return -1;
        }
        ...
        /* support the shortcut keys "setup", "expand" and "browser" */
            ...
            if(!down){
				String bsUrl = SystemProperties.get("ro.sw.defaultwebsite","http://v.qq.com");
				Log.v(TAG, "the default website is " + bsUrl);
                Uri uri = Uri.parse(bsUrl);
        ...
        if (isDefaultDisplay) {
            ...
            if(false){
			    if (navVisible || navAllowedHidden) {
                  ...
                  //we don't put up a fake input window to catch all touch events for homlet.
                  if (mHideNavFakeWindow != null) {
                       mHideNavFakeWindow.dismiss();
                       mHideNavFakeWindow = null;
                  }
    ...
    private DisplayManagerPolicy mDmPolicy;
    void setHdmiPlugged(boolean plugged) {
        if (mHdmiPlugged != plugged) {
            mDmPolicy.notifyDisplayDevicePlugedChanged(DisplayManager.DISPLAY_OUTPUT_TYPE_HDMI, plugged);
            ...
            intent.putExtra(DisplayManager.EXTRA_HDMISTATUS, plugged);
    ...
    void initializeHdmiState() {
        ...
        mDmPolicy = new DisplayManagerPolicy(mContext);
    ...
    void handleMute(){
        ...
        try {
            ...
            // that the music stops right before we call this
            mBroadcastWakeLock.acquire();
            audioService.adjustStreamVolume(AudioManager.STREAM_MUSIC, AudioManager.ADJUST_SAME, 0, mContext.getPackageName());
            audioService.setStreamMute(AudioManager.STREAM_MUSIC | AudioManager.SHOW_MUTE_UI, mIsMute = !mIsMute, mCallback);
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        if(DEBUG_BOOTFAST)
            Log.d(TAG,"mBootFastRuning = " + mBootFastRuning + " shutdhownPending = " + shutdhownPending);
        if (!mSystemBooted) {
        ...
        if(mBootFastEnable){
            if(mBootFastRuning||shutdhownPending){
                Log.d(TAG,"shutdown is running or boot animation running");
                return 0;
            }
            if(mPowerManager.isBootFastStatus()&&keyCode!=KeyEvent.KEYCODE_POWER){
                Log.d(TAG,"not power key return");
                return 0;
            }
            ...
            if(keyCode == KeyEvent.KEYCODE_POWER&&mPowerManager.isBootFastStatus()){
                if(!mPowerManager.isBootFastWakeFromStandby()){
                    if(down){
                        Log.d(TAG,"boot fast key down ");
                        interceptBootFastPowerKeyDown(isScreenOn);
                        return 0;
                    }else{
                        Log.d(TAG,"boot fast key up");
                    ...
                    if(down&&!mPowerManager.isScreenOn()){
                            Log.d(TAG,"wake from boot fast");
                            mHandler.removeCallbacks(mPowerLongPress);
                            mHandler.removeCallbacks(mBootFastPowerLongPress);
                            mHandler.post(mBootFastPowerLongPress);
                    }else{
                        if(DEBUG_BOOTFAST)
                            Log.d(TAG,"not light screen");
                        return 0;
                    }
                }
            }
...
//            case KeyEvent.KEYCODE_MUTE:
	...
	int uiVisibility=win.getSystemUiVisibility();
	if (uiVisibility==0 && win.getAttrs()!=null)
        {
            uiVisibility = win.getAttrs().subtreeSystemUiVisibility | win.getAttrs().systemUiVisibility;
            uiVisibility = uiVisibility & (View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_FULLSCREEN);
        }
        int tmpVisibility = uiVisibility
