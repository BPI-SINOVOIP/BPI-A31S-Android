...
public class StorageNotification extends SystemUI {
	...
	boolean usbConnectState(){
		boolean retval = false;
		try {
            IBinder b = ServiceManager.getService(Context.USB_SERVICE);
            IUsbManager service = IUsbManager.Stub.asInterface(b);
			retval = service.getCurrentmConnectState();
        } catch (Exception e) {
            Log.e(TAG, "Unable to get usb real time state", e);
        }
		return retval;
	}
	
	boolean usbConnectSettingIsUms(){
		boolean retval = false;
		retval = SystemProperties.get("sys.usb.state").indexOf("mass_storage")!=-1;
		return retval;
	}
	
	Thread mUsbthread = new Thread("SystemUI#StorageNotification") {
        @Override
        public void run() {
			Log.d(TAG,"SystemUI#StorageNotification start");
			int i = 20;
			while(--i>0){
				if(usbConnectState()==false&&usbConnectSettingIsUms()==true){
					onUsbMassStorageConnectionChangedAsync(false);
				}
				SystemClock.sleep(3000);
			}
			Log.d(TAG,"SystemUI#StorageNotification exit");
        }
    };
    @Override
    public void start() {
                ...
                if (path.equals(list[i])) {
                    if (path.contains("extsd")) {
                        ...
                        setMediaStorageNotification(
                                com.android.internal.R.string.ext_media_unmountable_notification_title,
                                com.android.internal.R.string.ext_media_unmountable_notification_message,
                                com.android.internal.R.drawable.stat_notify_sdcard_usb, true, true, pi);
