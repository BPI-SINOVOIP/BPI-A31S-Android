...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    if (mUmsSharingCount++ == 0) {
        ...
        if (MAJOR(d) == 93) //is nand
        {
        	  char nr_requests_path[256];
        	  char max_sectors_kb_path[256];
            mSavedNrRequests = -1; // in case we fail
            sprintf(nr_requests_path, "/sys/block/nand%c/queue/nr_requests", 'a' +( MINOR(d) >> 3));
            if ((fp = fopen(nr_requests_path, "r+"))) {
                char line[16];
                ...
                SLOGE("Failed to open %s (%s)", nr_requests_path, strerror(errno));
            }       
            mSavedMaxSectorsKb = -1; // in case we fail
            sprintf(max_sectors_kb_path, "/sys/block/nand%c/queue/max_sectors_kb", 'a' +( MINOR(d) >> 3));
            if ((fp = fopen(max_sectors_kb_path, "r+"))) {
                char line[16];
                ...
                SLOGE("Failed to open %s (%s)", max_sectors_kb_path, strerror(errno));
            }
        }
    }
    return 0;
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    if (MAJOR(d) == 93)
    {
        ...
        if (mUmsSharingCount == 0 && mSavedNrRequests != -1) {
            FILE* fp;
            sprintf(nr_requests_path, "/sys/block/nand%c/queue/nr_requests", 'a' +( MINOR(d) >> 3));
            if ((fp = fopen(nr_requests_path, "r+"))) {	
                fprintf(fp, "%d\n", mSavedNrRequests);
        ...
        if (mUmsSharingCount == 0 && mSavedMaxSectorsKb != -1) {
            FILE* fp;
            sprintf(max_sectors_kb_path, "/sys/block/nand%c/queue/max_sectors_kb", 'a' +( MINOR(d) >> 3));
            if ((fp = fopen(max_sectors_kb_path, "r+"))) {
                fprintf(fp, "%d\n", mSavedMaxSectorsKb);
