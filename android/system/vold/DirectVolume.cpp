...
int DirectVolume::handleBlockEvent(NetlinkEvent *evt) {
    ...
    //add by aw lisidong
    const char *type = evt->findParam("DEVNAME");
    //treat sr as cdrom
    if (!strncmp(type, "sr", strlen("sr"))) {
        mIsCDRom = true;
    } else {
        mIsCDRom = false;
    }

    SLOGD("handleBlockEvent mIsCDRom=%s type=%s" ,mIsCDRom?"true":"false", type);
    PathCollection::iterator  it;
    for (it = mPaths->begin(); it != mPaths->end(); ++it) {
        if (!strncmp(dp, *it, strlen(*it))) {
            ...
            if (action == NetlinkEvent::NlActionAdd) {
                ...
                if (major == 179) {
                    if (!mPartsChangeFlag) {
                        if (!strncmp(getMountpoint(), "/mnt/sdcard", strlen("/mnt/sdcard"))
