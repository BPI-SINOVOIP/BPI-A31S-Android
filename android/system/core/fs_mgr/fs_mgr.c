...
int setup_ext4(const char *blockdev,off64_t reserve)
{
    char buf[256]={0}, path[128]={0},fs_end_buf[30]={0};
    pid_t child;
    int status, n;
    off64_t fs_end=0;
    ...
    if (reserve>0)
    {
        ...
        if (fd>0)
        {
            fs_end=lseek64(fd,-reserve,SEEK_END);
            sprintf(fs_end_buf,"%lld",fs_end);
            close(fd);
        }
    }
tryagain:
    ...
    if (child == 0) {
        if (fs_end>0)
	    {
            ERROR("begin to format ext4 reserve : %lld ", reserve);
            execl(mkext4fs, mkext4fs,"-l", fs_end_buf ,buf, NULL);
        }
        else
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        if (fstab->recs[i].fs_mgr_flags & MF_CHECK) {
            check_fs(n_blk_device, fstab->recs[i].fs_type,
                     fstab->recs[i].mount_point,false,NULL,false);
        }
