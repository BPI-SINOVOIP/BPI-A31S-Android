...
static void export_kernel_boot_props(void)
{
    ...
    for (i=10;i<19;i++)
        cpuinfobuf[i]=cpuinfobuf[i+13];
	cpuinfobuf[ 19 ] = '\0';
	i = 0;
	property_set(prop_map[i].dest_prop, &(cpuinfobuf[0]));
...
#define ASYNC_INIT_SELINUX	1
#if ASYNC_INIT_SELINUX

static union selinux_callback cb;

// for async init selinux
static void * selinux_init_thread(void *thread_data)
{
