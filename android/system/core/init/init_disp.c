...
static int  init_disprequestlayerscreen(int fb_id,struct init_disppara_t *displaypara)
{
	...
	ioctl(mFD_disp,DISP_CMD_LAYER_GET_PARA,(void*)arg);
    ...
    ioctl(mFD_disp,DISP_CMD_LAYER_SET_PARA,(void*)arg);
...
__s32 is_support_av(__u32 sel)
{
    ...
    args[0] = sel;
    ret = ioctl(mFD_disp,DISP_CMD_TV_SUPPORT_FLAG,args);
...
int init_initdisplay()
{
    ...
    {
        //ERROR("#av flag = %d\n",is_support_av(0));
        if(is_support_av(0) > 0){
            init_swtichdisplay(0,DISP_OUTPUT_TYPE_TV,DISP_TV_MOD_PAL);
        }else{
            ERROR("##av is unsupported\n");
        }
    }
