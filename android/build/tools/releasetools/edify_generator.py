...
class EdifyGenerator(object):
  ...
  def MakePartitionSymlink(self):
    """adjust older PartitionSymlink"""
    fstab = self.info.get("fstab", None)
    prefix = "/dev/block/by-name/"
    prefixlen = len(prefix)
    if fstab:
      for k,v in fstab.items():
        node = v.device[prefixlen:];
        if (v.device.find(prefix) != -1):
          self.script.append(('if (!file_exists("/dev/block/by-name/%s")) '
              'then (symlink("/dev/block/%s", '
              '"/dev/block/by-name/%s")) endif;') % (node, node, node))
