...
#define debug ALOGV("LINE %d, FUNCTION %s", __LINE__, __FUNCTION__);
...
//                        ALOGD("(f:%s, l:%d) no output frame, wait <40ms for msg coming, interval=[%lld]ms", __FUNCTION__, __LINE__, (tm3-tm1)/1000);
				...
				//ALOGD("+++++++++++++++ picture.pts = %lld", picture.pts);
            	...
            	if(pSelf->m_useAndroidBuffer == OMX_FALSE)
            	{
                    ...
                    cedarx_cache_op((long)picture.y, (long)picture.y + picture.width * picture.height * 2, 0);
                    TransformToVirtualBuffer(&picture, pOutBufHdr->pBuffer);	// YUV420 planar
            	}
            	else
            	{
                    ...
                    //TransformToYUVPlaner(&picture, dst);
                    TransformToGPUBuffer(&picture, dst);

                	//if (!poutputFile) {
                	//	poutputFile = fopen("data/camera/decodeOutput.dat", "wb");
                	//}
                	//if (poutputFile) {
                	//	fwrite(dst, 1, 1280 * 720 *1.5, poutputFile);
                	//}
            ...
            {
            	ALOGV("LINE %d, nInBufEos %d, nVdrvNotifyEosFlag %d", __LINE__,  nInBufEos, nVdrvNotifyEosFlag);
...
__EXECUTE_CMD_DONE:
        ...
        if (pSelf->m_state == OMX_StateExecuting || pSelf->m_state == OMX_StatePause)
        {
			...
			else if(decodeResult == CEDARV_RESULT_NO_BITSTREAM)
			{
                if(nEosFlag)
                {
                    pSelf->m_decoder->ioctrl(pSelf->m_decoder, CEDARV_COMMAND_FLUSH, 0);
                    ...
                    ALOGV("(f:%s, l:%d) no_vbsinput_buffer, eos wait!", __FUNCTION__, __LINE__);
                    sem_wait(&pSelf->m_sem_vbs_input);
                    ALOGV("(f:%s, l:%d) no_vbsinput_buffer, eos wait_done!", __FUNCTION__, __LINE__);
                }
                else
                {
