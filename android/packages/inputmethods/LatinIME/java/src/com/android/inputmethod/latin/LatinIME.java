...
public class LatinIME extends InputMethodService implements KeyboardActionListener,
	...
	private MainKeyboardView mInputView;
	private int accuracy = 3;

	private boolean setFields(){
		mInputView = mKeyboardSwitcher.getMainKeyboardView();
		...
		if(mKeys != null){
			mCurKeyboardKeyNums = mKeys.length;
		}else{
	...
	private Key findNearestKey(int direction,Key srcKey){
		...
		int tmpY = 9999;
		Key retKey = null;
		Key[] nearestKeyIndices = null;


		switch(direction){
		case UP:
			nearestKeyIndices = mCurrentKeyboard.getNearestKeys(srcKey.getX(),srcKey.getY());
			for(i = 0; i < nearestKeyIndices.length; i++){
				Key nearKey = nearestKeyIndices[i];
	...
	public boolean onKeyDown(int keyCode, KeyEvent event){
		Log.d(TAG,"key event " + KeyEvent.keyCodeToString(keyCode));
		Key nearestKey = null;
		switch(keyCode){
		...
		case KeyEvent.KEYCODE_DPAD_CENTER:
			...
			mPreKey = mLastKey;
			final MotionEvent me = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_DOWN, mLastKey.getX(), mLastKey.getY(), 0);
			final MainKeyboardView tempInputView = mInputView;
...
*/	
    ...
    public boolean onKeyUp(final int keyCode, final KeyEvent event) {
		switch (keyCode) {
            ...
            final MotionEvent me = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_UP, mLastKey.getX(), mLastKey.getY(), 0);
            final MainKeyboardView tempInputView = mInputView;
            if (mPreKey.getCode() == Constants.CODE_ENTER) {
                mHandler.postDelayed (new Runnable(){
                    public void run(){
