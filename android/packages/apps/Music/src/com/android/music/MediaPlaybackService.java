...
public class MediaPlaybackService extends Service {
    ...
    private boolean isPrevShuffleNormal = false;
    private boolean isButtonNext = false;

    // stop handleMessage() after service onDestroy()
    ...
    private Handler mMediaplayerHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (MediaPlaybackService.this == null || MediaPlaybackService.this.isDestroy)
                return;
            switch (msg.what) {
                case FADEDOWN:
                    mCurrentVolume -= .05f;
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            mQuietMode = true;
            if(mPlayListLen > 0) {
                //If stop(false) is called in openCurrentAndNext,
                //then set mIsSupposedToBePlaying as false to set
                //button mPauseButton as paused, add by weihongqiang.
    ...
    public void prev() {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_NORMAL) {
                ...
                // when histsize == 0, make prev button not a no-op
                /*
                if (histsize == 0) {
                    // prev is a no-op
                    return;
                ...
                */
                ...
                if (histsize >= 2) {
                    mPlayPos = mHistory.get(histsize - 2);
                } else {        // histsize == 0 || histsize == 1
                    if (mPlayPos > 0) {
                        mPlayPos--;
                    } else {
    ...
    private int getNextPosition(boolean force) {
        boolean isLastPrevShuffleNormal = isPrevShuffleNormal;
        boolean isLastButtonNext = isButtonNext;
        isPrevShuffleNormal = false;
        isButtonNext = false;

        if (mRepeatMode == REPEAT_CURRENT && !isLastButtonNext) {
            ...
            // reasonable size
            if (mPlayPos >= 0 && !isLastPrevShuffleNormal) {
    ...
    public void buttonGotoNext(boolean force) {
        isButtonNext = true;
        gotoNext(force);
    }
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        public void buttonNext() {
            mService.get().buttonGotoNext(true);
        }
        public String getTrackName() {
