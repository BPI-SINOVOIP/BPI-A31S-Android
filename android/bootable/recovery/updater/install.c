...
Value* BurnBootFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    int flash_type;
    DeviceBurn burnFunc;
    flash_type = getDeviceInfo(boot_num, dev_path, boot_bin, &burnFunc);
    ...
    ZipArchive* za = ((UpdaterInfo*)(state->cookie))->package_zip;
    const ZipEntry* entry = mzFindZipEntry(za, boot_bin);
    if (entry == NULL) {
...
Value* FileExistsFn(const char* name, State* state, int argc, Expr* argv[]){
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
    }
    char* file_path;
    if (ReadArgs(state, argv, 1, &file_path) < 0) return NULL;
