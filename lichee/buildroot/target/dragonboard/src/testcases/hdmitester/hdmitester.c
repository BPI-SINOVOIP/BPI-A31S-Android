...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if((DISP_OUTPUT_TYPE_NONE != disp_output_type)
			&& (DISP_OUTPUT_TYPE_HDMI != disp_output_type))
		{
			db_warn("hdmitester: other display device running, output_type[%d]\n", disp_output_type);
			sleep(1);
			continue;
		}
		...
		if (ret == 1) {
			db_warn("hdmitester:HPD!\n");
			flags = 0;

			if (status == 1) {
				sleep(1);
				db_warn("hdmitester:status is 1!\n");
				if(disp_output_type == DISP_OUTPUT_TYPE_NONE){
				    args[0] = 0;
			...
			*/
			...
			retry = 0;
     	    ...
     	    if(DISP_OUTPUT_TYPE_HDMI == disp_output_type){
		    	args[0] = 0;
		    	ioctl(disp, DISP_CMD_HDMI_OFF, args);
            }
