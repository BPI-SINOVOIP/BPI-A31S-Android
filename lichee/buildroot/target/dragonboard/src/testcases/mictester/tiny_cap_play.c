...
int setup_mixer(int mixer_num)
{
    ...
    int r_input_source,l_input_source,ret;
   ...
   if (script_fetch("mic", "volume", &volume, 1) || 
	...
	ctl = mixer_get_ctl_by_name(mixer,"Speaker Function");	
	ret = mixer_ctl_set_value(ctl,0,1);
	if(ret < 0) {
		printf("mixer_ctl_set_value:failed!ret = %d\n",ret);
	}
	ret = mixer_ctl_set_enum_by_string(ctl,"spk_headset");
	if(ret < 0) {
		printf("mixer_ctl_set_enum_by_string:failed!ret = %d\n",ret);
	}
	/*
                            ...
                            unsigned int bits,int argc, char **argv)
{
	...
	ret = mixer_ctl_set_enum_by_string(ctl,"headset");
	if(ret < 0) {
		printf("mixer_ctl_set_enum_by_string:failed!ret = %d\n",ret);
	}
