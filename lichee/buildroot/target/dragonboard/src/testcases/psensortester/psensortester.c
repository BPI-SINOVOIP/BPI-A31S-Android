...
#define  PROXIMITY_THRESHOLD_G2A  5.0f
...
int main(int argc, char *argv[])
{
	...
	*/    
		...
		if ((event.type == EV_ABS) && (event.code == ABS_DISTANCE)) {
			x =event.value;
			distance = x * PROXIMITY_THRESHOLD_G2A;
			sprintf(buf,"(%4.2f)",distance);
