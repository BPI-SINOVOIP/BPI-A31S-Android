...
module_count=`script_fetch "psensor" "module_count"`
if [ $module_count -gt 0 ]; then
    for i in $(seq $module_count); do
        key_name="module"$i"_path"
        module_path=`script_fetch "psensor" "$key_name"`
