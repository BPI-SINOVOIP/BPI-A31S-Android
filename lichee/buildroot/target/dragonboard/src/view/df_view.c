...
int df_reflash_scn_win(void)
{
	...
	unsigned long int scale_factor = 100;
	...
	args[0] = 0;
    output_type = (__disp_output_type_t)ioctl(disp, DISP_CMD_GET_OUTPUT_TYPE,(void*)args);
	if(output_type == disp_output_type)
	{
	    goto out;
	}
...
//    db_error("####layer_para.src_win.width = %d,layer_para.src_win.height = %d\n",layer_para.src_win.width,layer_para.src_win.height);
//    db_error("####layer_para.scn_win.width = %d,layer_para.scn_win.height = %d\n",layer_para.scn_win.width,layer_para.scn_win.height);

    switch(disp_output_type){
	    case DISP_OUTPUT_TYPE_HDMI:
		    if(0==script_fetch("df_view", "hdmi_scale_factor", &scale_factor,1)){
			    if((scale_factor<50) || (scale_factor>100)){
			        db_msg("the hdmi_scale_factor isn't correct,set it to defult:no scale.\n");
			        scale_factor=100;
			    }else{
			        db_msg("fetch hdmi_scale_factor successfully,hdmi_scale_factor=%ld\n", scale_factor);
			    }
			}
			layer_para.scn_win.width = (unsigned int)(1280 * scale_factor * 0.01);
			layer_para.scn_win.height = (unsigned int)(720 * scale_factor * 0.01);
			layer_para.scn_win.x = (unsigned int)(1280 - layer_para.scn_win.width) >> 1; // ((100 - scale_factor) * 1280 * 0.01) >> 1;
			layer_para.scn_win.y = (unsigned int)(720 - layer_para.scn_win.height) >> 1; //((100 - scale_factor) * 720 * 0.01) >> 1;
			break;

		case DISP_OUTPUT_TYPE_TV:
		    if(0==script_fetch("df_view", "tv_scale_factor", &scale_factor,1)){
                if((scale_factor<50) || (scale_factor>100)){
                    db_msg("the tv_scale_factor isn't correct,set it to defult:no scale.\n");
                    scale_factor=100;
                }else{
                    db_msg("fetch tv_scale_factor successfully,tv_scale_factor=%ld\n", scale_factor);
                }
			}
			layer_para.scn_win.width = (unsigned int)(690 * scale_factor * 0.01);
			layer_para.scn_win.height = (unsigned int)(546 * scale_factor * 0.01);
			layer_para.scn_win.x = (unsigned int)(30 * scale_factor * 0.01) + (690 - layer_para.scn_win.width) >> 1; //30 + ((100 - scale_factor) * 690 / 100 >> 1);
			layer_para.scn_win.y = (unsigned int)(30 * scale_factor * 0.01) + (546 - layer_para.scn_win.height) >> 1; //30 + ((100 - scale_factor) * 546 / 100 >> 1);
			break;

		default:
		    db_msg("not deal the output_tpye[%d] yet!\n", disp_output_type);
		    break;
	}
	{
...
#if 0 //packed into df_reflash_scn_win()
    /* open /dev/disp */
    if ((disp = open("/dev/disp", O_RDWR)) == -1) {
        db_error("can't open /dev/disp(%s)\n", strerror(errno));
        return -1;
...
#else
    df_reflash_scn_win();
#endif //packed into df_reflash_scn_win()
